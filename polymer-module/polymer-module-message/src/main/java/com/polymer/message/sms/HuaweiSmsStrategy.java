...
public class HuaweiSmsStrategy implements SmsStrategy {
    ...
    public void send(String mobile, Map<String, String> params) {
        ...
        if (StringUtils.isBlank(wsseHeader)) {
            throw new ServiceException("wsse header is null.");
        }
        ...
        try {
            ...
            if (status == HttpStatus.OK.value()) {
                ...
                // 短信是否发送成功
                if (result == null) {
                    throw new ServiceException("短信发送返回内容为空！");
                } else if (!"000000".equals(result.code)) {
                    throw new ServiceException(result.description);
                }
            } else { //400 401
                throw new ServiceException(IOUtils.toString(connection.getErrorStream(), StandardCharsets.UTF_8));
            }
        } catch (Exception e) {
    ...
    static String buildWsseHeader(String appKey, String appSecret) {
        if (null == appKey || null == appSecret || appKey.isEmpty() || appSecret.isEmpty()) {
            throw new ServiceException("buildWsseHeader(): appKey or appSecret is null.");
        }
        SimpleDateFormat sdf = new SimpleDateFormat(DateUtils.UTC_PATTERN);
        String time = sdf.format(LocalDateTime.now());
        String nonce = TokenUtils.generator();

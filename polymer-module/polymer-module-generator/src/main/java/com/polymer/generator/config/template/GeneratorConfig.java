package com.polymer.generator.config.template;
...
public class GeneratorConfig {
    ...
    public GeneratorInfo getGeneratorConfig() {
        ...
        if (isConfig == null) {
            throw new ServiceException("模板配置文件，config.json不存在,template:"+ template);
        }
        ...
        try {
            ...
            for (TemplateInfo templateInfo : generator.getTemplates()) {
                ...
                if (isTemplate == null) {
                    throw new ServiceException("模板文件 " + templateInfo.getTemplateName() + " 不存在");
                }
                // 读取模板内容

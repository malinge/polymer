...
public class ProjectUtils {
    ...
    public static void contentFormat(File rootFile, List<String> suffixList, Map<String, String> replaceMap) {
        List<File> destList = FileUtils.loopFiles(rootFile, file -> suffixList.contains(FileTypeUtils.getSuffix(file)));

        for (File dest : destList) {
            List<String> lines = FileUtils.readUtf8Lines(dest);
            ...
            FileUtils.writeUtf8Lines(newList, dest);
        }
    }
    ...
    public static String getTmpDirPath(String... names) {
        StringBuilder tmpPath = new StringBuilder(FileUtils.getTmpDirPath());
